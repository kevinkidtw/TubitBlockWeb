...
| **Linux** | 核心已內建 `ch341` 模組，**無需額外安裝**。 |

> ⚠️ **提醒**：建議資訊教師於學期初統一在電腦教室映像檔中預裝驅動，避免學生上課時才發現無法連線。
...
2. **(首次使用必做) 安裝硬體驅動程式**：點擊網頁右上角的「齒輪（設定）」圖示，並選擇「安裝驅動」，系統將自動下載 CH341SER 官方 USB 驅動程式，請執行該 `.exe` 檔完成安裝。
